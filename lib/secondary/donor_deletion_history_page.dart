...
class DonorDeletionHistoryPage extends StatefulWidget {
  @override
  _DonorDeletionHistoryPageState createState() => _DonorDeletionHistoryPageState();
}

class _DonorDeletionHistoryPageState extends State<DonorDeletionHistoryPage> {
  List<Person> _deletedDonors = [];
  ...
  Future<void> _fetchDeletedDonors() async {
    try {
      setState(() {
        _isLoading = true;
        _errorMessage = null;
      ...
      setState(() {
        _deletedDonors = deletedItems;
        _isLoading = false;
      });
    } catch (e) {
      setState(() {
        _errorMessage = 'Failed to load donor deletion history: ${e.toString()}';
        _isLoading = false;
      });
    }
  }
  ...
  Widget _buildHistoryList() {
    ...
    if (_deletedDonors.isEmpty) {
      return _buildEmptyStateUI();
    }

    return ListView.builder(
      itemCount: _deletedDonors.length,
      itemBuilder: (context, index) {
        final person = _deletedDonors[index];
        return _DonorDeletionHistoryTile(person: person);
      },
    );
  }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text(
            'No donors have been deleted yet.',
            style: TextStyle(
              fontFamily: "Inter",
              fontSize: 14,
              color: Colors.grey,
...
class _DonorDeletionHistoryTile extends StatelessWidget {
  final Person person;
  const _DonorDeletionHistoryTile({required this.person});
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            Text(
              "Deleted: ${person.timestamp != null ? _formatTimestampHistory(person.timestamp) : 'Unknown Date'}",
              style: const TextStyle(
                fontSize: 10,
                fontFamily: "Inter",
                fontWeight: FontWeight.w400,
